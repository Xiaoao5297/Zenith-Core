<?php namespace ZMusicBox; use pocketmine\command\Command; use pocketmine\command\CommandExecutor; use pocketmine\command\CommandSender; use pocketmine\plugin\PluginBase; use pocketmine\event\Listener; use pocketmine\level; use pocketmine\Server; use pocketmine\scheduler\PluginTask; use pocketmine\utils\Config; use pocketmine\utils\TextFormat; use pocketmine\network\protocol\BlockEventPacket; use pocketmine\Player; use pocketmine\math\AxisAlignedBB; use pocketmine\math\Vector3; use pocketmine\math\Math; use pocketmine\level\format\Chunk; use pocketmine\level\format\FullChunk; use pocketmine\utils\BinaryStream; use pocketmine\utils\Binary; use ZMusicBox\NoteBoxAPI; 
class ZMusicBox extends PluginBase implements Listener{
	public $song;
	public $MusicPlayer;
	public $name;
	public function onEnable(){
		$this->getLogger()->info("ZMusicBox正在启动...");
		...
		$this->getServer()->getPluginManager()->registerEvents($this,$this);
		if(!$this->CheckMusic()){
			$this->getLogger()->info("§b请将nbs文件放到song文件夹下");
		}else{
			$this->StartNewTask();
		}
		$this->getLogger()->info("ZMusicBox启动完成...");
	} 
			...
			case "music":
				if(isset($args[0])){
					switch($args[0]){
						case "next":
							$this->StartNewTask();
							$sender->sendMessage(TextFormat::GREEN."[音乐盒子] 播放下一首歌曲.");
							return true;
							break;
						case "stop":
							if($sender->isOp()){
								$this->getServer()->getScheduler()->cancelTasks($this);
								$sender->sendMessage(TextFormat::GREEN."[音乐盒子] 停止音乐.");
							}else{
								$sender->sendMessage(TextFormat::RED."[音乐盒子] 你没有权限使用此命令!");
							}
							return true;
							break;	
						case "start":
							if($sender->isOp()){
								$this->StartNewTask();
								$sender->sendMessage(TextFormat::GREEN."[音乐盒子] 音乐播放.");
							}else{
								$sender->sendMessage(TextFormat::RED."[音乐盒子] 你没有权限使用此命令!");
							}
							return true;
					...
					$sender->sendMessage(TextFormat::RED."§astart-播放音乐\n§astop-停止音乐\n§anext-播放下一首音乐");
				}
			break;		
		}
	}
	...
	public function Play($sound,$type = 0,$blo = 0){
		if(is_numeric($sound) and $sound > 0){
			foreach($this->getServer()->getOnlinePlayers() as $p){
				...
				if(!empty($noteblock)){
					if($this->song->name != ""){
						$p->sendPopup("§6<<- 正在播放: §a".$this->song->name."§6->>");
					}else{	
						$p->sendPopup("§6<<- 正在播放: §a".$this->name."§6->>");
					}
					$i = 0;
					while ($i < $blo){
	...
	public function onDisable(){
		$this->getLogger()->info("MusicBox卸载成功!");
	}
