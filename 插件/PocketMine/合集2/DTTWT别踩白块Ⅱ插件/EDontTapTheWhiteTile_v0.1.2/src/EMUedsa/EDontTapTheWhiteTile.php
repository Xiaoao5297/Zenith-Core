...
class EDontTapTheWhiteTile extends PluginBase implements Listener{
	...
	public function onLoad(){
		$this->getLogger()->info(TextFormat::GREEN . "[别踩白块] 插件正在加载 !");
	}
	...
	public function onEnable(){
		...
		$this->getLogger()->info(TextFormat::DARK_GREEN . "[别踩方块] 插件已开启 ! 本插件由 极致·人生 美化修改 + 流星 优化！");
    }

	public function onDisable(){
		$this->getLogger()->info(TextFormat::DARK_RED . "[别踩白块] 插件已关闭 !");
	}
				...
				switch ($args[0]) {
					case 'set':
						if($this->config->getAll() !==  array()){
							$sender->sendMessage("§c[别踩白块] 游戏已经设置,请输入/dtt del 来删除游戏设置!");
						}else{
							$name = $sender->getName();
							$this->SetStatus[$name] = 0;
							$sender->sendMessage("§b[别踩白块] 你已进入设置状态,请点击4×5方框的左下角和右上角和四个木牌!");
						}
						break;
					...
					case 'del':
						...
						unset($this->Top2);
						unset($this->Top3);
						$sender->sendMessage("§b[别踩白块] 成功清除当前游戏设置!");
						break;
		...
		if(isset($this->SetStatus[$username])){
			switch ($this->SetStatus[$username]) {
				case 0:
					...
					$player->sendMessage(TextFormat::GREEN."§b[ 别踩白块] 方框的左下角已设置完成 x=".$block->x." y=".$block->y." z=".$block->z." 游戏所在地图:".$levelname);
					$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 请点击方框的右上角.");
					break;
				case 1:
					if($this->pos1["level"] === $levelname AND $this->pos1["x"] == $block->x){
						if(abs($this->pos1["y"] - $block->y) == 4 AND abs($this->pos1["z"] - $block->z) == 3){
							...
							$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 方框的右上角已设置完成 x=".$block->x." y=".$block->y." z=".$block->z." 游戏所在地图:".$levelname);
							$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 请分别点击4个木牌作为游戏显示器.");
						}else{
							$this->SetStatus[$username] = 0;
							$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 方框的右上角已设置完成 x=".$block->x." y=".$block->y." z=".$block->z." 游戏所在地图:".$levelname);
							$player->sendMessage(TextFormat::RED."§c[别踩白块] 请确认设置是在同一地图的 4 × 5 的方框. \n§b[别踩白块] 请重新设置一二点!");
						}
				...
				case 2:
					if($block->getId() == 63 OR $block->getId() == 68 OR $block->getId() == 323){
						...
						$player->getLevel()->getTile($StartSignVector3)->setText("§b别踩白块","§b点击开始","§b当前玩家: 无","");
						$this->SetStatus[$username]++;
						$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 开始木牌设置成功!");
						$player->sendMessage(TextFormat::RED."§b[别踩白块] 请点击木牌作为排行榜木牌1.");
					}else{
						$player->sendMessage(TextFormat::RED."§b[别踩白块] 请点击木牌作为游戏开始按钮.");
					}
					break;
				case 3:
					if($block->getId() == 63 OR $block->getId() == 68 OR $block->getId() == 323){
						...
						$player->getLevel()->getTile($top1Vector3)->setText("§b第一名","","999","");
						$this->SetStatus[$username]++;
						$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 排行榜木牌1设置成功!");
						$player->sendMessage(TextFormat::RED."§b[别踩白块] 请点击木牌作为排行榜木牌2.");
					}else{
						$player->sendMessage(TextFormat::RED."§b[别踩白块] 请点击木牌作为排行榜木牌1.");
					}
					break;
				case 4:
					if($block->getId() == 63 OR $block->getId() == 68 OR $block->getId() == 323){
						...
						$player->getLevel()->getTile($top2Vector3)->setText("§b第二名","","999","");
						$this->SetStatus[$username]++;
						$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 排行榜木牌2设置成功!");
						$player->sendMessage(TextFormat::RED."§b[别踩白块] 请点击木牌作为排行榜木牌3.");
					}else{
						$player->sendMessage(TextFormat::RED."§b[别踩白块] 请点击木牌作为排行榜木牌2.");
					}
					break;
				case 5:
					if($block->getId() == 63 OR $block->getId() == 68 OR $block->getId() == 323){
						...
						$player->getLevel()->getTile($top3Vector3)->setText("§b第三名","","999","");
						unset($this->SetStatus[$username]);
						$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 排行榜木牌3设置成功!");
						Server::getInstance()->broadcastMessage(TextFormat::YELLOW." §b[别踩白块] 游戏设置已完成,可以进行游戏了!");
					}else{
						$player->sendMessage(TextFormat::RED."§b[别踩白块] 请点击木牌作为排行榜木牌3.");
					}
					break;
				default:
					$player->sendMessage(TextFormat::RED."§c[别踩白块] 发生了未知错误!");
					break;
			}
			...
			if($this->line[0][0]["level"] === $levelname AND $this->line[0][0]["x"] <= $block->x AND $this->line[0][0]["y"] <= $block->y AND $this->line[0][0]["z"] <= $block->z AND $this->line[4][3]["x"] >= $block->x AND $this->line[4][3]["y"] >= $block->y AND $this->line[4][3]["z"] >= $block->z){
				if($block->y == $this->line[0][0]["y"]){
					if($block->getId() == 35 AND $block->getDamage() == 0){
						...
						$player->sendMessage(TextFormat::RED."§c[别踩白块] 你失败了,游戏结束!");
						...
						if($this->GameStart[$username] == 50){
							...
							$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 恭喜你成功过关,游戏结束,成绩: ".$gametime." 秒!");
							...
							if($top1Text[2] > $gametime){
								$top1sign->setText("§b第一名",$username,$gametime,"");
								Server::getInstance()->broadcastMessage(TextFormat::YELLOW."§b[别踩白块] 恭喜".$username."赢得了 别踩白块 小游戏第一名,用时: ".$gametime." 秒!");
							}else{
								...
								if($top2Text[2] > $gametime){
									$top2sign->setText("§b第二名",$username,$gametime,"");
									Server::getInstance()->broadcastMessage(TextFormat::YELLOW."§b[别踩白块] 恭喜".$username."赢得了 别踩白块 小游戏第二名,用时: ".$gametime."秒!");
								}else{
									...
									if($top3Text[2] > $gametime){
										$top3sign->setText("§b第三名",$username,$gametime,"");
										Server::getInstance()->broadcastMessage(TextFormat::YELLOW."§b[别踩白块] 恭喜".$username."赢得了 别踩白块 小游戏第三名,用时 : ".$gametime."秒!");
									}else{
										Server::getInstance()->broadcastMessage(TextFormat::YELLOW."§b[别踩白块] 恭喜".$username."完成了 别踩白块 小游戏,用时: ".$gametime."秒!");
									}
								}
							}
							$this->getServer()->getScheduler()->cancelTasks($this);
				...
				if(($block->getId() == 63 OR $block->getId() == 68 OR $block->getId() == 323) AND $block->x == $this->StartSign["x"] AND $block->y == $this->StartSign["y"] AND $block->z == $this->StartSign["z"] AND $levelname === $this->StartSign["level"]){
					if(isset($this->GameStart) AND !isset($this->GameStart[$username])){
						$player->sendMessage(TextFormat::RED."§c[别踩白块] 有玩家正在游戏中,你无法加入游戏!");
					}elseif(isset($this->GameStart[$username])){
						unset($this->GameStart);
						$c = 1;
						for($i=0;$i<5;$i++){
						...
						$StartSignVector3 = new Vector3($this->StartSign["x"],$this->StartSign["y"],$this->StartSign["z"]);
						$player->getLevel()->getTile($StartSignVector3)->setText("§b别踩白块","§b点击开始","§b当前玩家: 无","");
						$player->sendMessage(TextFormat::RED."§c[别踩白块] 游戏取消!");
					}
				}
			}
		}else{
			if(isset($this->line[0][0]["level"])){
				if(($block->getId() == 63 OR $block->getId() == 68 OR $block->getId() == 323) AND $block->x == $this->StartSign["x"] AND $block->y == $this->StartSign["y"] AND $block->z == $this->StartSign["z"] AND $levelname === $this->StartSign["level"]){
					if(isset($this->GameStart)){
						...
						$this->GameStart[$username]++;
						$StartSignVector3 = new Vector3($this->StartSign["x"],$this->StartSign["y"],$this->StartSign["z"]);
						$player->getLevel()->getTile($StartSignVector3)->setText("§b别踩白块","§b游戏进行中","§b当前玩家:".$username,"");
						$player->sendMessage(TextFormat::GREEN."§b[别踩白块] 游戏开始,请点击黑色方块!");
					}
				}
			}
		}
	...
	public function GameTimeout(){
		unset($this->GameStart);
		Server::getInstance()->broadcastMessage(TextFormat::YELLOW."§c[别踩白块] 游戏超时!");
		$c = 1;
		$level = $this->getServer()->getLevelByName($this->line[0][0]["level"]);
		$StartSignVector3 = new Vector3($this->StartSign["x"],$this->StartSign["y"],$this->StartSign["z"]);
		$level->getTile($StartSignVector3)->setText("§b别踩白块","§b点击开始","§b当前玩家: 无","");
		for($i=0;$i<5;$i++){
			for($n=0;$n<4;$n++){
				$pos = new Vector3($this->line[$i][$n]["x"],$this->line[$i][$n]["y"],$this->line[$i][$n]["z"]);
				if($c % 2 == 0){
