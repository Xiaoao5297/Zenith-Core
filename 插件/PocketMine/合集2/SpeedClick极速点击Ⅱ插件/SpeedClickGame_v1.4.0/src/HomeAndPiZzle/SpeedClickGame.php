...
//use pocketmine\scheduler\CallbackTask;
use HomeAndPiZzle\Task\CallbackTask;
class SpeedClickGame extends PluginBase implements Listener
{
	public function onEnable(){
        ...
        $this->getLogger()->info(CL::GREEN."| 极速点击Ⅱ 插件加载完成，本插件由极致·人生优化 |");
	...
	public function jishi(){
		if($this->gamestart){
			...
			switch($this->shijian2){
				...
				$this->gamestart = false;
				$this->cx = 4;
				$p11->sendTip(CL::GREEN."§9[极速点击Ⅱ] 游戏结束!");
				$p11->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 你一共点击了".($this->css)."次.");
				$this->players = array();
				$this->css = 0;
				$this->csign();
				break;
				...
				if($s->isOp()){
				    switch($args[0]){
						...
						if(!$this->cfg->exists("sign")){
						    $s->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 已进入设置状态,请先设置状态木牌!");
							$this->cx = 1;
						}else{
							$s->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 游戏已设置.");
						}
						break;
						case"remove":
						if(!$this->cfg->exists("sign")){
							$s->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 游戏未设置.");
						}else{
							$s->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 删除成功!");
							$this->cx = 0;
							$this->gamestart = false;
							$this->css = 0;
							$this->shuliang = 0;
					...
					$s->sendMessage(CL::GREEN."§c[极速点击Ⅱ] 你没有权限执行此操作!");
				}
			}
		}
	}
		...
		switch($this->cx){
			...
			if($p->isOp()){
				...
				if($sign instanceof Sign){
				    $p->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 状态木牌已设置,请点击排行榜木牌.");
				    $this->sign111 = array(
					    "x" =>$xx,
						"y" =>$yy,
						"z" =>$zz,
					...
					$p->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 请点击一个方块作为游戏方块.");
				}
			}
			break;
			case"2":
			if($p->isOp()){
				$sign=$e->getPlayer()->getLevel()->getTile($e->getBlock());
				if($sign instanceof Sign){
				    $p->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 排行榜木牌已设置完成,请点击一个方块作为游戏方块.");
					$sign->setText("§9[极速点击Ⅱ]","§9最高纪录","§9当前玩家: 无" ,"");
				    $this->sign112 = array(
					    "x" =>$xx,
						"y" =>$yy,
						"z" =>$zz,
			...
			case"3":
			if($p->isOp()){
				$p->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 游戏设置已完成,可以开始游戏了!");
			...
			if(!$this->gamestart){
				if($this->cfg->exists("sign")){
					...
					if($xx == $x1 and $yy = $y1 and $zz == $z1 and $level->getFolderName() == $leveln){
						$p->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 你已加入游戏!");
					    $this->gamestart = true;
			...
			if($this->gamestart){
				...
				if($xx == $x1 and $yy = $y1 and $zz == $z1 and $level->getFolderName() == $leveln){
					if($pn == $this->players['id']){
						$this->css++;
						$p->sendMessage(CL::GREEN."§9[极速点击Ⅱ] 你一共点了".($this->css)."次.");
					}
				}
			}
			break;
		...
		if($this->cfg->exists("sign")){
			...
			if($sign instanceof Sign){
				switch(count($this->players)){
					case"0":
					$sign->setText("§9[极速点击Ⅱ]","§9点击加入","§9当前玩家: 无" ,"");
					break;
					case"1":
					$sign->setText("§9[极速点击Ⅱ]","§9游戏开始","§9当前玩家:".($this->players['id']) ,"");
					break;
				}
			}
			unset($sign);
		...
		if($this->cfg->exists("sign1")){
			...
			if($sign instanceof Sign){
				if($this->css > $signtext[3]){
					$sign->setText("§9[极速点击Ⅱ]","§9最高纪录","§9玩家:".($this->players['id']) ,($this->css));
					Server::getInstance()->broadcastMessage(CL::YELLOW."§9[极速点击Ⅱ] ".($this->players['id'])."获得了极速点击Ⅱ第一名!");
				}
			}
