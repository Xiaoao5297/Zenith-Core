...
Server::getInstance()->broadcastMessage("§a[清理系统] 共清理{$i}个掉落物.");
	...
	public function removeMobs(){
        ...
        Server::getInstance()->broadcastMessage("§a[清理系统] 共清理{$i}个生物.");
    }
...
class cleaner extends PluginTask
{
    ...
    public function onRun($currentTick)
    {
    	...
    	if($this->tmp==100)
    	{
    		Server::getInstance()->broadcastMessage("§a[清理系统] 将在5秒后清除所有掉落物和生物.");
    	}
    	if($this->tmp==200)
    	{
    		Server::getInstance()->broadcastMessage("§a[清理系统] 将在10秒后清除所有掉落物和生物.");
    	}
    	if($this->tmp==400)
    	{
    		Server::getInstance()->broadcastMessage("§a[清理系统] 将在20秒后进行世界清理.");
    	}
    }
