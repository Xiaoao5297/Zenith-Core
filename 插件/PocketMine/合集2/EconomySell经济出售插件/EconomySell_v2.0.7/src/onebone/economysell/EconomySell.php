...
class EconomySell extends PluginBase implements Listener {
	...
	public function onEnable(){
		$this->getLogger()->info("§b经济回收商店  §d汉化by Mr_sky");
	...
	private function prepareLangPref(){
		$this->lang = new Config($this->getDataFolder()."language.properties", Config::PROPERTIES, array(
				"wrong-format" => "§b[ 回收商店]§e请按正确的格式写牌子",
				"item-not-support" => "§b[ 回收商店]§e物品 %1 不支持在回收商店中回收",
				"no-permission-create" => "§b[ 回收商店]§e你没有权限创建经济回收商店",
				"sell-created" => "§b[ 回收商店]§e回收商店成功创建 (%1 = %MONETARY_UNIT%%2)",
				"removed-sell" => "§b[ 回收商店]§e回收商店成功移除",
				"creative-mode" => "§b[ 回收商店]§e创造哪里凉快哪里去",
				"no-permission-sell" => "§b[ 回收商店]§e你没有权限将物品给回收商店回收",
				"no-permission-break" => "§b[ 回收商店]§e你没有权限移除回收商店",
				"tap-again" => "§b[ 回收商店]§e你确定让物品 %1 (%MONETARY_UNIT%%2 金币)出售? 再点击一次就可出售",
				"no-item" => "§b[ 回收商店]§e你已经没有物品可卖了",
				"sold-item" => "§b[ 回收商店]§e你卖出了 %1  %2 个，得到了 %MONETARY_UNIT%%3 金币"
		));

		$this->sellSign = new Config($this->getDataFolder()."SellSign.yml", Config::YAML, array(
				"回收" => array(
						"§b[ 回收商店]",
						"§e价格: %MONETARY_UNIT%%1 金币",
						"§f商品: %2",
						"§b数量: %3 个"
				)
		));
