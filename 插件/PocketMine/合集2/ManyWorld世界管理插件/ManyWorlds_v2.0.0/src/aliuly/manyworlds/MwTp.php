...
class MwTp extends BasicCli {
	public function __construct($owner) {
		...
		$this->enableSCmd("tp",["usage" => mc::_("[玩家id] <世界名字>"),
										"help" => mc::_("传送到其他世界"),
	...
	public function onSCommand(CommandSender $c,Command $cc,$scmd,$data,array $args) {
		...
		if (!$this->owner->autoLoad($c,$wname)) {
			$c->sendMessage(TextFormat::RED.mc::_("传送失败"));
			return true;
		}
		$level = $this->owner->getServer()->getLevelByName($wname);
		if ($level === null) {
			$c->sendMessage(TextFormat::RED.mc::_("取得世界名字出错%1%"));
			return true;
		}
		if ($c != $player) {
			$player->sendMessage(TextFormat::YELLOW.mc::_("正在把你从世界%2%传送到", $wname, $c->getName()));
		} else {
			$c->sendMessage(TextFormat::GREEN.mc::_("正在传送到 %1%",$wname));
		}
