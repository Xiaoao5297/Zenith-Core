...
class essentialsTP extends PluginBase  implements CommandExecutor, Listener {
        ...
        public function onPlayerDeath(PlayerDeathEvent $event)
    {
      ...
      $swd = $player->getLevel()->getName();
            $con = $this->confi->getall();
    if(!isset($con[$swd]) or $player->hasPermission("essentialstp.command.setwarp")){
    ...
    public function onCommand(CommandSender $sender, Command $cmd, $label, array $args){
        switch($cmd->getName()){
            ...
            case 'home':
                ...
                break;
               case 'addswd':
			   	if(!isset($args[0])){$sender->sendMessage("请输入地图名");}
            $con = $this->confi->getall();
    if($sender->hasPermission("essentialstp.command.setwarp")){
    if(isset($args[0])){
    $a = $args[0];
    if(!isset($con[$a])){
    $this->confi->set($a,array());
    $sender->sendMessage("已添加 $a ");
    $this->confi->save();}}return true;
    ...
    if(isset($con[$a])){
unset($con[$a]);
    $this->confi->setall($con);
    $sender->sendMessage("已经取消 $a ");
    $this->confi->save();}}return true;
    }else{$sender->sendMessage("你没有权限");}
    break;
            case 'sethome':
            $swd = $sender->getLevel()->getName();
            $con = $this->confi->getall();
    if(!isset($con[$swd]) or $sender->hasPermission("essentialstp.command.setwarp")){
                if ($sender instanceof Player)
                {
                ...
                }else{$sender->sendMessage("此地图不能设置家");}return true;
                break;
            case 'delhome':
                if ($sender instanceof Player)
                {
    ...
    if(!isset($con[$swd]) or $sender->hasPermission("essentialstp.command.setwarp")){
                ...
                else
                {
                    $sender->sendMessage(TextFormat::RED.$this->config->get("Lang_command_only_use_ingame"));
                    return true;
                }}else{$sender->sendMessage("此地图不能tpa");}return true;
                break;
            case 'tpahere':
         $swd = $sender->getLevel()->getName();
            $con = $this->confi->getall();
    ...
    public function create_db(){
        ...
        if ($count == 0){
            $this->prepare = $this->db2->prepare("CREATE TABLE homes (
                      ...
                      world TEXT)");
            $this->result = $this->prepare->execute();
            $this->getLogger()->info(TextFormat::AQUA."essentialsTP+ 家数据创建!");
        }
        ...
        if ($count2 == 0){
            $this->prepare = $this->db2->prepare("CREATE TABLE tp_requests (
                      ...
                      status TEXT)");
            $this->result = $this->prepare->execute();
            $this->getLogger()->info(TextFormat::AQUA."essentialsTP+ 请求数据创建!");
        }
        ...
        if($count3 == 0){
            $this->prepare = $this->db2->prepare("CREATE TABLE warps (
                      ...
                      title TEXT)");
            $this->result = $this->prepare->execute();
            $this->getLogger()->info(TextFormat::AQUA."essentialsTP+ 传送点数据创建!");
        }
        ...
        if($count4 == 0){
            ...
            $this->result = $this->prepare->execute();
            $this->getLogger()->info(TextFormat::AQUA."essentialsTP+ 地图出生点数据创建!");
        }
        ...
        if($count5 == 0){
            ...
            $this->result = $this->prepare->execute();
            $this->getLogger()->info(TextFormat::AQUA."essentialsTP+ 冷却时间数据创建!");
        }
    ...
    public function check_config(){
        ...
        $this->confi = new Config($this->getDataFolder()."world.yml", Config::YAML, array());
        ...
        {
            $this->config->set("MOTD", "EssintialsTP+ 欢迎你使用配置文件里的服务器公告");
            $this->config->save();
        }
    ...
    public function onEnable(){
        $this->getLogger()->info(TextFormat::GREEN."essentialsTP+ 加载...");
        ...
        $this->create_db();
        $this->getLogger()->info(TextFormat::GREEN."[INFO] 加载 [".TextFormat::GOLD."config.yml".TextFormat::GREEN."]....");
        $this->tpa_cooldown = time() - $this->config->get("tpa-here-cooldown");
        $this->getLogger()->info(TextFormat::GREEN."[INFO] 加载 [".TextFormat::GOLD."config.yml".TextFormat::GREEN."] DONE");
        $this->getLogger()->info(TextFormat::GREEN."essentialsTP+ 开启!");
    ...
    public function onDisable(){
        $this->prepare->close();
        $this->getLogger()->info("essentialsTP+ 关闭");
    }
