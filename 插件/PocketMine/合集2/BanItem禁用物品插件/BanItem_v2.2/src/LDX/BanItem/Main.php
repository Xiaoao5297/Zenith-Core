...
class Main extends PluginBase implements Listener {
  ...
  public function onEnable() {
	$this->getLogger()->info("§b新版禁用物品插件  §d汉化by Mr_sky");
    $this->saveItems();
  ...
  public function onTouch(PlayerInteractEvent $event) {
    ...
    if($this->isBanned($event->getItem())) {
      if(!($p->hasPermission("banitem") || $p->hasPermission("banitem.bypass"))) {
        $p->sendMessage("[BanItem] 这个物品被禁用了!");
        $event->setCancelled();
      }
    }
  }
  ...
  public function onCommand(CommandSender $p,Command $cmd,$label,array $args) {
    ...
    if(strtolower($args[0]) == "ban" || strtolower($args[0]) == "unban") {
      ...
      if(!is_numeric($item[0]) || (isset($item[1]) && !is_numeric($item[1]))) {
        $p->sendMessage("[BanItem] §c请输入物品的ID以禁用!");
        return true;
      }
    }
    if(strtolower($args[0]) == "ban") {
      ...
      if(in_array($i,$this->items)) {
        $p->sendMessage("[BanItem] §c这个物品已经被禁用了.");
      } else {
        array_push($this->items,$i);
        $this->saveItems();
        $p->sendMessage("[BanItem] §a这个物品 " . str_replace("#",":",$i) . " 已添加到禁用列表中.");
      }
      ...
      if(!in_array($i,$this->items)) {
        $p->sendMessage("[BanItem] §c这个物品没有被禁用.");
      } else {
        array_splice($this->items,array_search($i,$this->items),1);
        $this->saveItems();
        $p->sendMessage("[BanItem] §a这个物品 " . str_replace("#",":",$i) . " 已从禁用列表中删除.");
      }
    } else if(strtolower($args[0]) == "list") {
      $p->sendMessage("[BanItem] §e禁用列表" . (count($this->items) == 1 ? "" : "") . ": §f" . str_replace("#", ":", implode(", ", $this->items)) . (count($this->items) > 0 ? "." : "§7None."));
    } else {
      return false;
    }
    return true;
