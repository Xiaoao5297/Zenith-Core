...
class SimpleAuth implements Plugin{
	...
	public function init(){
		...
		if(file_exists($this->api->plugin->configPath($this)."players.yml")){			
			...
			console("[信息] [身份验证] Importing old format to new format...");
		...
		foreach($this->sessions as $CID => $timer){
			if($timer !== true and $timer !== false and isset($this->server->clients[$CID]) and ($this->server->clients[$CID] instanceof Player)){				
				if($broadcast === true){
					...
					if($d === false){					
						$this->server->clients[$CID]->sendChat("[身份验证] 你必须使用 /register <password> 进行注册");
					}else{
						$this->server->clients[$CID]->sendChat("[身份验证] 你必须使用 /login <password> 进行登陆");
					}
				}
		...
		switch($event){
			...
			case "player.spawn":
				if(!isset($this->sessions[$data->CID]) or $this->sessions[$data->CID] !== true){
					...
					$data->blocked = true;
					$data->sendChat("[身份验证] 该服务器使用身份验证来保护你的帐户.");
					if($this->config->get("allowRegister") !== false){
						...
						if($d === false){					
							$data->sendChat("[身份验证] 你必须使用 /register <password> 进行注册");
						}else{
							$data->sendChat("[身份验证] 你必须使用 /login <password> 进行登陆");
						}
					}
				}
				break;
			...
			case "console.command":
				if(($data["issuer"] instanceof Player) and $this->sessions[$data["issuer"]->CID] !== true){
					if($data["cmd"] === "login" and $this->checkLogin($data["issuer"], implode(" ", $data["parameters"])) === true){
						...
						$data["issuer"]->sendChat("[身份验证] 密码太短.");
						return true;
						...
						$data["issuer"]->sendChat("[身份验证] 注册成功.");
						$this->login($data["issuer"]);
						return true;
					}elseif($data["cmd"] === "login" or $data["cmd"] === "register"){
						$data["issuer"]->sendChat("[身份验证] 身份验证错误.");
						return true;
					}
					return false;
				}
