...
class WorldEditor implements Plugin{
	...
	public function setPosition1(&$session, Position $position, &$output){
		...
		$output .= "第一个方位设置在这里！ (".$session["selection"][0][0].", ".$session["selection"][0][1].", ".$session["selection"][0][2].")$count.\n";
		return true;
	}
	...
	public function setPosition2(&$session, Position $position, &$output){
		...
		$output .= "第二个方位设置在这里！ (".$session["selection"][1][0].", ".$session["selection"][1][1].", ".$session["selection"][1][2].")$count.\n";
		return true;
	}
